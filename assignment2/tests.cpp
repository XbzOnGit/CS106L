...
void A_copy_ctor_basic() {
    ...
    for (const auto& kv_pair : vec) {
        //std::cout << kv_pair.first << std::endl << kv_pair.second << std::endl << std::endl;
        map.insert(kv_pair);
        answer.insert(kv_pair);
    }
    //map.debug();
...
void F_move_edge() {
    ...
    }
    {
        ...
        HashMap<std::string, int> map;
        std::unordered_map<std::string, int> answer;
        //std::cout << "Testing self-move" << std::endl;
        map = std::move(map);
