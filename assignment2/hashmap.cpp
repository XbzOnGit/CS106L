...
{
    std::size_t bucketnum = rhs.size();
    //std::cout << bucketnum << std::endl;
    for(std::size_t i = 0; i < bucketnum; ++i)
    {
        node* curr = rhs[i];
        if(curr != nullptr)
        {
            node* consprev = new node(curr->value, nullptr);
            this->_buckets_array[i] = consprev;
            curr = curr->next;
            while(curr != nullptr)
            {
                node* tempnew = new node(curr->value, nullptr);
                consprev->next = tempnew;
                consprev = tempnew;
                curr = curr->next;
            }
        }
    }
    //std::cout << "After copying the vector!!" << std::endl << std::endl;
    //this->debug();
}

template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const HashMap& rhs):_size(rhs._size),_hash_function(rhs._hash_function),_buckets_array(rhs._buckets_array.size(),nullptr)
{
    this->copy_bucket_arr(rhs._buckets_array);
}
template <typename K, typename M, typename H>
typename HashMap<K, M, H>::HashMap& HashMap<K, M, H>::operator=(const HashMap& rhs)
{
    if(this == (&rhs))
    {
        return (*this);
...
template <typename K, typename M, typename H>
typename HashMap<K, M, H>::HashMap& HashMap<K, M, H>::operator=(HashMap&& rhs)
{
    //Just ignore self-move.  
    if(this == (&rhs))
    {
        return (*this);
    }
...
void HashMap<K, M, H>::clear() {
    for (auto& curr : _buckets_array) {
        //Here we should use a reference.  
        //curr is an alias for _buckets_array[i], with the type of node*  
        while (curr != nullptr) {
            auto fdel = curr;
